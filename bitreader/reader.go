...
func (r *Reader) Read(bitSize int64) ([]uint8, uint8, error) {
	if r.isEnded {
		return nil, 0, io.EOF
	}
	...
	for remain != 0 {
		...
		if read == 0 {
			if err := r.nextByte(); err != nil {
				...
				return result, totalRead, fmt.Errorf("Reader.Read: %w", err)
			}
			continue
		}
...
func (r *Reader) Peek(bitSize int64) ([]uint8, uint8, error) {
	offset := r.offset
	...
	result, read, err := r.Read(bitSize)
	if err != nil {
		return nil, 0, fmt.Errorf("Reader.Peek: %w", err)
	}
